...
const { removeEmptyColumns } = require('../util/helper')
 ...
 */
module.exports = function (moduleName, moduleAst, opts) {
  ...
  while (len--) {
    ...
    switch (type) {
      case 'Number':
        defualtText = md.code(defaultVal) || '-'
        break
      case 'Array':
        defaultText =
  ...
  // need remove empty colums
  if (opts.removeEmptyColumns) {
    contentTables = removeEmptyColumns(tableHeader, tableBody, opts.emptyFlags)
  }

  let content = md.table(contentTables)
